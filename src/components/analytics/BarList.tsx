    ...
    <div className={cn('flex flex-col gap-2', className)}>
      {items.map((item, i) => (
        <div key={item.key ?? i} className="flex items-center gap-3">
          <div className={cn('w-24 shrink-0 truncate text-xs text-ink-dim', labelClassName)}>
            {item.label}
          </div>
          ...
          <div className="w-14 shrink-0 text-right font-mono text-xs tabular-nums text-ink">
