            ...
            {errorsOpen && (
              <ul className="mt-1.5 flex flex-col gap-1 border-l border-danger/30 pl-3">
                ...
                {errors.length > ERROR_COUNT && (
                  <li className="font-mono text-[11px] tabular-nums text-ink-faint">
              ...
              <tbody>
                {examples.slice(0, SAMPLE_COUNT).map((e) => (
                  <tr key={e.id} className="border-b border-hairline/60 last:border-b-0">
                    ...
                    <td className="max-w-0 truncate px-2 py-1.5 text-xs text-ink">
                      {firstUserContent(e) || (
                        <span className="italic text-ink-faint">no user turn</span>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-right font-mono text-xs tabular-nums text-ink-dim">
                      {e.messages.length}
                    </td>
                  </tr>
                ))}
