      ...
      {entries.map((entry) => (
        <div key={entry.id} className="flex flex-col gap-3">
          <div className="panel flex items-center gap-2.5 px-3 py-2">
            ...
            <span className="min-w-0 truncate font-mono text-xs text-ink">{entry.name}</span>
            ...
            <span className="ml-auto flex min-w-0 shrink-0 items-center gap-1.5">
              ...
              {(entry.status === 'parsing' ||
                ...
                <>
                  ...
                  <span className="text-[11px] text-ink-dim">{PHASE_LABELS[entry.status]}</span>
                </>
              )}
              {entry.status === 'done' && (
                <>
                  <CheckCircle2 className="size-3.5 text-ok" aria-hidden />
                  <span className="text-[11px] text-ok">Ready</span>
                </>
              )}
              {entry.status === 'failed' && (
                <>
                  <CircleAlert className="size-3.5 shrink-0 text-danger" aria-hidden />
                  <span className="max-w-72 truncate text-[11px] text-danger" title={entry.error}>
