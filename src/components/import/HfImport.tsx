      ...
      {showResults && (
        <div className="panel overflow-hidden">
          {search.isLoading ? (
            ...
            <p className="flex items-center gap-1.5 px-3 py-2.5 text-xs text-danger">
              <CircleAlert className="size-3.5 shrink-0" aria-hidden />
              Search failed. Check your connection.
            </p>
          ) : searchResults.length === 0 ? (
            <p className="px-3 py-2.5 text-xs text-ink-faint">No datasets found.</p>
          ) : (
              ...
              {searchResults.map((d) => (
                <li key={d.id}>
                  ...
                  >
                    ...
                    <span className="flex shrink-0 items-center gap-1 font-mono text-[11px] tabular-nums text-ink-faint">
                      <Download className="size-3" aria-hidden /> {fmtNum(d.downloads)}
                    </span>
                    <span className="flex shrink-0 items-center gap-1 font-mono text-[11px] tabular-nums text-ink-faint">
                      <Heart className="size-3" aria-hidden /> {fmtNum(d.likes)}
                    </span>
                  </button>
                </li>
              ))}
        ...
        <section className="panel animate-rise" aria-label="Selected dataset">
          <div className="panel-header">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate font-mono text-xs text-ink">{selected.id}</span>
            </div>
            <Button
              variant="ghost"
              size="icon"
          ...
          <div className="flex flex-col gap-3 p-3">
            {info.isLoading && (
              <div className="flex items-center gap-2 text-xs text-ink-dim">
                <Spinner className="size-3" /> Loading dataset info…
              </div>
            )}
            {info.isError && (
            ...
            {info.data && (
              <>
                ...
                {config && split && (
                  <div className="rounded-(--radius-control) border border-hairline bg-surface-2">
                    ...
                    {rowsPreview.isLoading ? (
                      <div className="flex items-center gap-2 px-2.5 py-2 text-xs text-ink-dim">
                        <Spinner className="size-3" /> Loading rows…
                      </div>
                    ) : rowsPreview.isError ? (
                      <p className="flex items-center gap-1.5 px-2.5 py-2 text-xs text-danger">
                        <CircleAlert className="size-3.5 shrink-0" aria-hidden />
                      ...
                      <ul>
                        {rowsPreview.data?.rows.map((row, i) => (
                          <li
                            key={i}
                            className="truncate border-b border-hairline/60 px-2.5 py-1.5 font-mono text-[11px] text-ink-dim last:border-b-0"
                          >
                            {rowSnippet(row)}
                          </li>
                        ))}
                ...
                <p className="text-[11px] text-ink-faint">0 imports everything.</p>
