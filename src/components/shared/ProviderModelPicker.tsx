...
export function ProviderModelPicker({
  ...
  if (configs && enabled.length === 0) {
    return (
      <div className="rounded-(--radius-control) border border-dashed border-hairline px-3 py-2.5 text-xs text-ink-dim">
        No AI provider configured.{' '}
        <Link to="/settings" className="inline-flex items-center gap-1 text-accent hover:underline">
          <Settings2 className="size-3" /> Add one in Settings
        </Link>
