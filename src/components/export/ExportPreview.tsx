    ...
    <section className={cn('panel', className)} style={style}>
      ...
      <div className="p-3">
        {preview.unsupported !== null ? (
          <div className="flex items-center justify-center gap-2 py-8 text-xs text-ink-dim">
          ...
          <Tabs key={pathKey} defaultValue={files[0]?.path}>
            <TabsList className="h-auto flex-wrap justify-start">
              {files.map((file) => (
                <TabsTrigger key={file.path} value={file.path} className="font-mono text-[11px]">
                  {file.path}
                </TabsTrigger>
              ))}
            </TabsList>
            {files.map((file) => (
              <TabsContent key={file.path} value={file.path} className="mt-2">
                {typeof file.content === 'string' ? (
                  <pre className="max-h-96 overflow-auto whitespace-pre rounded-(--radius-control) border border-hairline bg-surface-2 p-3 font-mono text-[11px] leading-relaxed text-ink-dim">
                    {file.content.length > MAX_PREVIEW_CHARS
                      ? `${file.content.slice(0, MAX_PREVIEW_CHARS)}\n… preview truncated`
                      : file.content}
                  </pre>
                ) : (
                  <p className="px-1 py-2 font-mono text-[11px] tabular-nums text-ink-faint">
