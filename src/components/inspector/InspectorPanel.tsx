    ...
    >
      <div className="panel-header">
        <div className="flex min-w-0 items-center gap-2">
          <span className="tech-label">Example</span>
          <Tip label={<span className="font-mono text-[10px]">{example.id}</span>}>
        ...
        <div className="flex items-center gap-0.5">
          ...
          <Tip label={example.flagged ? 'Remove flag' : 'Flag for review'}>
            <Button
              variant="ghost"
              size="icon"
              className={cn('h-6 w-6', example.flagged && 'text-danger hover:text-danger')}
          ...
          <Tip label={example.reviewed ? 'Mark as not reviewed' : 'Mark as reviewed'}>
            <Button
              variant="ghost"
              size="icon"
              className={cn('h-6 w-6', example.reviewed && 'text-ok hover:text-ok')}
      ...
      <div className="flex flex-col gap-1.5 border-b border-hairline px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="h-6 w-28 px-2 text-[11px]" aria-label="Split">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="flex flex-wrap items-center gap-1">
          {example.tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 rounded-(--radius-control) border border-hairline bg-surface-2 px-1.5 py-px font-mono text-[10px] text-ink-dim"
            >
              {tag}
              <button
                type="button"
          ...
          >
            <Input
              ...
              placeholder="Add tag"
              aria-label="Add tag"
              className="h-5.5 w-24 px-1.5 font-mono text-[10px]"
            />
          </form>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-t border-hairline px-3 py-2">
        {dirty && <span className="text-[11px] text-ink-dim">Unsaved changes</span>}
