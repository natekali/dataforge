...
function CommandRow({ label, command }: { label: string; command: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <code
        className="min-w-0 flex-1 truncate rounded-(--radius-control) border border-hairline bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] text-ink-dim"
        title={command}
      >
        {command}
      </code>
        ...
        <div className="space-y-3 p-3">
          ...
          <div>
            <button
              ...
              className="flex items-center gap-1 text-xs text-ink-dim transition-colors duration-100 hover:text-ink"
            >
              <ChevronRight
                className={cn('size-3 transition-transform duration-100', advancedOpen && 'rotate-90')}
              />
              Advanced
            </button>
            {advancedOpen && (
