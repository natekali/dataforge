...
export function AnalyticsPage() {
  ...
  const splitItems: BarListItem[] = SPLITS.map((s) => ({
    key: s,
    label: <span className="font-mono text-[11px] uppercase tracking-wide">{s}</span>,
  ...
  const roleItems: BarListItem[] = ROLE_ORDER.filter(
    ...
    key: r,
    label: <span className="font-mono text-[11px]">{r}</span>,
  ...
  const qualityItems: BarListItem[] = QUALITY_BUCKETS.map((b, i) => ({
    ...
    label: (
      <span className="flex items-baseline gap-1.5">
        ...
        <span className="font-mono text-[10px] tabular-nums text-ink-faint">{b.range}</span>
      </span>
    ),
  ...
  const tagItems: BarListItem[] = stats.topTags.map((t) => ({
    key: t.tag,
    label: <span className="truncate font-mono text-[11px] text-ink-dim">{t.tag}</span>,
    value: t.count,
  }));
          ...
          <section className="panel animate-rise" style={rise()}>
            ...
            <div className="p-3">
              {roleItems.length > 0 ? (
                ...
                <p className="py-2 text-xs text-ink-faint">No messages yet.</p>
              )}
            </div>
          </section>
        </div>
